...
module DivisionEuclidienne
  ...
  let div_euclidienne (a b: int) : (int, int)
    requires { a >= 0 ∧ b > 0 }
    ensures { result = (quotient, remainder) ∧ a = b * quotient + remainder ∧ remainder >= 0 ∧ remainder < b }
  =
    let
      ...
      -- Invariant de boucle I : a = b * quotient + remainder
      invariant { a = b * quotient + remainder ∧ remainder >= 0 ∧ remainder < b }

      -- Programme p : soustraction successive
      loop_body =
        -- Condition de sortie : remainder < b
        if remainder < b then
          -- Sortie : (quotient, remainder)
          (quotient, remainder)
        else
          -- Corps de la boucle : soustraction
          let
            new_remainder = remainder - b
            new_quotient = quotient + 1
          in
            -- Mise à jour des variables
            { quotient = new_quotient; remainder = new_remainder }
    in
      -- Application de la boucle while (remainder >= b)
      while (remainder >= b) loop_body
end
```
...
module DivisionEuclidienne
  ...
  =
    let
      ...
      -- Invariant de boucle I : a = b * quotient + remainder ∧ remainder >= 0 ∧ remainder < b
      invariant { a = b * quotient + remainder ∧ remainder >= 0 ∧ remainder < b }
