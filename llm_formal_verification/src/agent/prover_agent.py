...
class AgenticProver:
    ...
    def __init__(self, llm: LocalLLM, rag: RAGSystem, verifier: Why3Verifier):
        ...
        # ENGLISH SYSTEM PROMPT + CHEAT SHEET v2.1
        # Small models (2B) are sensitive to noise. ASCII-only is mandatory for Why3.
        self.system_prompt = (
            ...
            "### RULE #1: ASCII ONLY ###\n"
            "NEVER use Unicode math symbols. Use Why3-legal ASCII operators ONLY:\n"
            "- AND:  `&&`  or  `/\` \n"
            "- OR:   `||`  or  `\/` \n"
            "- IMPLIES: `->` \n"
            "- NOT:  `not` \n\n"
            "### RULE #2: STATE MANAGEMENT ###\n"
            "- For SIMPLE functions (no loops): Use `let` bindings only. Do NOT use `ref`.\n"
            "- For LOOP functions: Use `ref` for variables that change inside the loop. Use `!var` for read and `var := val` for write.\n\n"
            "### WHY3 SYNTAX SAMPLES ###\n"
            "module Example\n"
            "  use int.Int\n"
            "  use ref.Ref (* Only for loops *)\n\n"
            "  (* SIMPLE FUNCTION *)\n"
            "  let f (x: int) : int\n"
            "    requires { x >= 0 }\n"
            "    ensures  { result >= x }\n"
            ...
            "2. Modules names MUST be Capitalized (e.g., `module MaxFunc`).\n"
            "3. MANDATORY ATTRIBUTION: Cite your source as a comment (e.g., `(* Derived from: Course01 *)`)."
        )
